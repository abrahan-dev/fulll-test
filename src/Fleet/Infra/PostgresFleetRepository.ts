...
export class PostgresFleetRepository implements FleetRepository {
    ...
    async search(id: FleetId): Promise<Fleet | null> {
        const resultFleets = await sql`SELECT id, name, user_id FROM fleets WHERE id = ${id.getValue()}`.values();
        const resultVehicles = await sql`SELECT fleet_id, vehicle_id FROM fleet_vehicles WHERE fleet_id = ${id.getValue()}`.values();
        ...
        return new Fleet(
            new FleetId(row[0]),
            new FleetName(row[1]),
            resultVehicles.length === 0 ? [] : resultVehicles[0].map((vehicleId: string) => new VehicleId(vehicleId)),
            new UserId(row[2])
        );
    }
}
